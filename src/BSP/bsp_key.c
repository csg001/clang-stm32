...
*	版    本 : V1.3
*	说    明 :
*扫描独立按键，具有软件滤波机制，具有按键FIFO。可以检测如下事件： (1) 按键按下
*				(2) 按键弹起
*				(3) 长按键
*				(4) 长按时自动连发
*
...
*		V1.2	2016-01-25 armfly  针对P02工控板更改.
*调整gpio定义方式，更加简洁 V1.3	2018-11-26 armfly  s_tBtn结构赋初值0
*
...
*	函 数 名: IsKeyDownFunc
*	功能说明:
*判断按键是否按下。单键和组合键区分。单键事件不允许有其他键按下。 形    参: 无
*	返 回 值: 返回值1 表示按下(导通），0表示未按下（释放）
*********************************************************************************************************
*/
...
void bsp_SetKeyParam(uint8_t _ucKeyID, uint16_t _LongTime,
  ...
  s_tBtn[_ucKeyID].RepeatSpeed =
      _RepeatSpeed; /* 按键连发的速度，0表示不支持连发 */
